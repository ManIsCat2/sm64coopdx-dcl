...
ALIGNED8 const u8 texture_ping_full[] = {
#include "textures/segment2/custom_ping_full.rgba16.inc.c"
};

ALIGNED8 const u8 texture_ping_full_one_off[] = {
#include "textures/segment2/custom_ping_four.rgba16.inc.c"
};

ALIGNED8 const u8 texture_ping_full_two_off[] = {
#include "textures/segment2/custom_ping_three.rgba16.inc.c"
};

ALIGNED8 const u8 texture_ping_full_three_off[] = {
...
static struct DjuiImage *djuiImages2[MAX_PLAYERS] = { 0 };
...
u8 sPageIndex = 0;
static u8 sPlayer = 0; // all player slots always exist this switches their visibility on and off if
...
static void playerlist_update_row(u8 i, struct NetworkPlayer *np) {
    ...
    char sActNum[7];
    if (!configShowPing) {
        djui_base_set_visible(&djuiImages2[i]->base, false);
        ...
        djui_base_set_visible(&djuiImages2[i]->base, true);
        snprintf(sActNum, 7, "%s", " ");
    }
    if (charIndex >= CT_MAX) {
        charIndex = 0;
    }
    ...
    #define and &&
    if (np->ping < 150) {
        djuiImages2[i]->texture = texture_ping_full;
    } else if (np->ping > 150 and np->ping < 300) {
        djuiImages2[i]->texture = texture_ping_full_one_off;
    } else if (np->ping > 300 and np->ping < 450) {
        djuiImages2[i]->texture = texture_ping_full_two_off;
    } else if (np->ping > 450 and np->ping < 600) {
        djuiImages2[i]->texture = texture_ping_full_three_off;
    }
    ...
    djui_base_set_color(&djuiTextDescriptions[i]->base, np->descriptionR, np->descriptionG,
                        np->descriptionB, np->descriptionA);
...
void djui_panel_playerlist_create(UNUSED struct DjuiBase *caller) {
    ...
    for (u8 i = 0; i < MAX_PLAYERS; i++) {
        ...
        struct DjuiImage *i2 = djui_image_create(&row->base, texture_ping_full, 16, 16, 8);
